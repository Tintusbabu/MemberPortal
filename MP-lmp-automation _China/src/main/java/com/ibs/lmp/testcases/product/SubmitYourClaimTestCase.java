...
public class SubmitYourClaimTestCase extends ExtentReportUtils {

    /**
     * Product_TC_006 -  Claim submission Page
     * @throws Exception
     */
    @Test(groups={"Product_TestCases"})
    public void ProductClaimSubmission_TC_009() throws Exception{
        ...
        SubmitYourClaimPage submitYourClaimPage = null;
        ...
        try{
            ...
            List<TestData> claimTestDataList = CommonUtility.getTestDataListForTestCase("ClaimSubmissionData", testCaseName);
            for(TestData testData: testDataList) {
                if(testCaseName.equals(testData.getTestCaseName())) {
                    ...
                    homePage.selectSubMenu("Claim Points","Submit Your Claim");
                    logger = loggerRoot.createNode("Submit Your Claim");
                    submitYourClaimPage= new SubmitYourClaimPage(driver, logger);
                    submitYourClaimPage.verifySubmitYourClaimPageTitle();
                    for(TestData claimData: claimTestDataList) {
                        if (testCaseName.equals(claimData.getTestCaseName())) {
                            submitYourClaimPage.claimSumbission(claimData);
                        }
                    }
                   // referralActivityPage.sendReferral(testData);
                    //ERROR-4641
