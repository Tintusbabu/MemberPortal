...
import com.ibs.lmp.pages.ReferralActivityPage;
 ...
 * @author A-4641
 * 
 * This is the test script to trigger the referral mail 
 * 
 * Revision History
 * 
 ...
 *  
 */
public class ReferralActivityTestCase extends ExtentReportUtils {

	
	/**
	 * Product_TC_007 - Send referral
	 * @throws Exception
	 */
	@Test (groups={"Product_TestCases"})
	public void Product_TC_007() throws Exception{
		...
		ReferralActivityPage referralActivityPage = null;
		try{
			...
			for(TestData testData: testDataList) {
				if(testCaseName.equals(testData.getTestCaseName())) {
					...
					homePage.selectSubMenu("Member","Referral");
					logger = loggerRoot.createNode("Referral");
					referralActivityPage= new ReferralActivityPage(driver, logger);
					referralActivityPage.verifyReferralActivityTitle();
